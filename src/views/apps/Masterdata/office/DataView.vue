<template>
    <v-card flat color="card">
        <v-card-title>Master Office</v-card-title>
        <v-card-text>
            ...
            <v-data-table dense flat show-expand item-key="id" :expanded.sync="expanded" :headers="headers" :items="desserts" :options.sync="options"
                ...
                <template v-slot:[`item.act`]="{ item }">
                    <v-icon small class="mr-2" @click="editItem(item.id)" v-if="$can('masteroffice-update')">
                        mdi-pencil
                    </v-icon>
                    <v-icon small @click="deleteItem(item.id)" v-if="$can('masteroffice-delete')">
                        mdi-delete
                    </v-icon>
                </template>
            </v-data-table>
...
export default {
    ...
    methods: {
        ...mapActions("masterdata_office", ['index', 'edit', 'delete']),
        getDataFromApi() {
            this.loading = true
