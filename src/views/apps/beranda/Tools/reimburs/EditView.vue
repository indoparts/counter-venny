        ...
        <v-card class="card" flat>
            <v-card-text>
                <v-row no-gutters>
                    ...
                    <v-col cols="12" md="3">
                        <v-file-input prepend-inner-icon="mdi-camera" dense outlined v-model="form.file_receipt"
                            class="mb-input" accept="image/*" label="File"></v-file-input>
                    </v-col>
                </v-row>
            </v-card-text>
            <v-card-text>
                <v-img :src="baseUrl + img"></v-img>
            </v-card-text>
            <v-card-actions>
...
export default {
    data: () => ({
        baseUrl: `http://${process.env.BASE_URL_API}/api/images/reimburs/`,
        loading: false,
        menu: false,
        img:'',
    ...
    created() {
        ...
        this.editReimburs(this.$route.params.id).then((res)=>{
            this.img = res.data.file_receipt
        })
    },
    methods: {
        ...mapActions('pengajuan_reimburs', ['editReimburs', 'updateReimburs', 'user']),
        submit() {
            this.loading = true
            this.updateReimburs(this.$route.params.id).then((res) => {
                this.$swal({
