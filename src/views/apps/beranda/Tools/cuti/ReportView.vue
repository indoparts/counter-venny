            ...
            <v-card-text>
                <v-data-table dense flat show-expand :headers="headers" :items="desserts" :options.sync="options"
                    ...
                    <template v-slot:[`item.btn_approval`]="{ item }">
                        <v-btn v-if="authenticated.id === item.user_id_approval" class="ma-2" outlined fab color="primary"
                            x-small @click="approvalact(item.id, item.user.name)">
...
export default {
    data() {
        return {
            ...
            options: {},
            headers: [
                { text: 'Mulai Tanggal', value: 'date' },
                { text: 'Sampai Dgn Tanggal', value: 'todate' },
    ...
    methods: {
        ...mapActions('pengajuan_cuti', ['reportExport', 'report', 'user', 'approval']),
        getDataFromApi() {
            this.loading = true
        ...
        approvalact(id, nama) {
            this.$swal({
                title: `Permintaan persetujuan permohonan cuti`,
                text: `Permintaan persetujuan permohonan cuti atas nama ${nama}`,
                showDenyButton: true,
                ...
                if (result.isConfirmed) {
                    this.approval({ id: id, status_approval: 'y' }).then((res) => {
                        if (res.status === true)
                            this.$swal('Tersimpan!', `Permohonan cuti atas nama ${nama} telah diizinkan`, 'success')
                        this.getDataFromApi()
                    })
                } else if (result.isDenied) {
                    this.approval({ id: id, status_approval: 'n' }).then((res) => {
                        if (res.status === true)
                            this.$swal('Tersimpan!', `Permohonan cuti atas nama ${nama} tidak diperizinkan`, 'info')
                        this.getDataFromApi()
                    })
                }
            })
                    ...
                    this.reportExport(this.tanggal).then((res) => {
                        ...
                        res.data.forEach(el => {
                            arr.push({
                                ...
                                "Permintaan Cuti": el.leave_req,
                                "Catatan Cuti": el.notes,
                                "Saldo Cuti": el.remaining,
                                "Jenis Cuti": el.req_type,
                                "Approval Cuti": el.status_approval === 'y' ? 'ya' : 'tidak',
                        ...
                        xlsx.writeFile(wb, 'cuti-data-exported.csv')
                        this.$swal(
                            'exported!',
